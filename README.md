...
Python Web Directory Scanner (pwds) — это многопоточный рекурсивный сканер веб-директорий написанный на Python с визуализацией структуры корневой веб-директории и автокалибровкой ложных ответов.



![Демонстрация работы](img/output3.png)

## Особенности
**Рекурсивное сканирование**: Добавляет найденные веб-директории в очередь сканирования в пределах заданной глубины рекурсии (опциональный флаг `--depth`).
...
--no-recursion  Отключает рекурсию при сканировании
--user-agent    Устанавливает `User-Agent` для сканирования
--proxy         Перенаправляет все запросы через указанный прокси
```
...
# 3. Запуск
python pwds.py -t 192.168.0.1 -w wordlist.txt
```
